...
class beamline_layer:
    ...
    def propagate_with_HC_veto(self, particle: particle):
        ...
        particle.variance_angle_rad = particle.variance_angle_rad + MCS_angle_kick**2
        if (
            math.sqrt(particle.variance_width_cm) > 3
            and self.material == "Scintillator"
        ):
            print("Beam width larger than HC hole size, slimming...")
            particle.variance_width_cm = 9
...
class beamline_setup:
    ...
    def create_setup(self, all_layers: list):
        self.layers.extend(
            beamline_layer(
                material=material, thickness_cm=thickness, density_gcm3=density
            )
            for material, thickness, density in all_layers
        )
    ...
    def get_final_variance_HC_veto(self):
